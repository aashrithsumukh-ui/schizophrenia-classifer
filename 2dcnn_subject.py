...
class BrainSliceDataset(Dataset):
    ...
    def __init__(self, subject_df: pd.DataFrame, transform=None,
        ...
        # Build flat index: list of (filepath, slice_idx, label, subject_id)
        self.samples = []
        for _, row in subject_df.iterrows():
            for s in range(slice_start, slice_end):
                self.samples.append((row["filepath"], s, int(row["label"]), str(row["subject_id"])))

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        filepath, slice_idx, label, subject_id = self.samples[idx]
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    for imgs, labels, _ in loader:  # _ = subject_ids, not needed during training
...
def evaluate(model, loader, criterion, device):
    ...
    for imgs, labels, _ in loader:  # _ = subject_ids, not needed during validation
        imgs, labels = imgs.to(device), labels.to(device)
        logits = model(imgs)
        loss   = criterion(logits, labels)
...
def get_predictions(model, loader, device):
    """Return true labels, predicted labels, positive-class probabilities, and subject IDs."""
    model.eval()
    all_labels, all_preds, all_probs, all_subjects = [], [], [], []
    ...
    return (np.array(all_labels), np.array(all_preds),
            np.array(all_probs), np.array(all_subjects))
...
def subject_level_metrics(labels, probs, subject_ids):
    """
    Aggregate slice-level predictions to subject level.
    Two strategies:
      - Mean probability: average P(SCHZ) across all slices per subject, threshold at 0.5
      - Majority vote: count how many slices predicted SCHZ, majority wins
    Both should give similar results; mean-prob AUC is the more informative one.
    """
    df = pd.DataFrame({
    ...
    subj = df.groupby("subject_id").agg(
        true_label    = ("label",  "first"),       # label is same for all slices
        mean_prob     = ("prob",   "mean"),         # mean P(SCHZ) across slices
        majority_vote = ("pred",   lambda x: int(x.mean() >= 0.5)),  # majority vote
    ).reset_index()

    # ── Mean-probability aggregation ──────────────────────────
    subj_preds_mean = (subj["mean_prob"] >= 0.5).astype(int)
    acc_mean  = accuracy_score(subj["true_label"], subj_preds_mean)
    auc_mean  = roc_auc_score(subj["true_label"], subj["mean_prob"])
    cm_mean   = confusion_matrix(subj["true_label"], subj_preds_mean)
    tn, fp, fn, tp = cm_mean.ravel()
    sens_mean = tp / (tp + fn)
    spec_mean = tn / (tn + fp)

    # ── Majority-vote aggregation ──────────────────────────────
    acc_maj  = accuracy_score(subj["true_label"], subj["majority_vote"])
    cm_maj   = confusion_matrix(subj["true_label"], subj["majority_vote"])
    tn2, fp2, fn2, tp2 = cm_maj.ravel()
    sens_maj = tp2 / (tp2 + fn2)
    spec_maj = tn2 / (tn2 + fp2)

    n_subj = len(subj)
    print("\n" + "="*55)
    print("     SUBJECT-LEVEL EVALUATION  (n={})".format(n_subj))
    print("="*55)
    print("  [Mean Probability Aggregation]")
    print(f"  Accuracy    : {acc_mean:.4f}  ({acc_mean*100:.2f}%)")
    print(f"  AUC-ROC     : {auc_mean:.4f}")
    print(f"  Sensitivity : {sens_mean:.4f}  (SCHZ recall)")
    print(f"  Specificity : {spec_mean:.4f}  (CTRL recall)")
    print(f"\n  Confusion Matrix (Mean Prob, threshold=0.5):")
    print(f"                Pred CTRL  Pred SCHZ")
    print(f"  Actual CTRL:    {tn:5d}      {fp:5d}")
    print(f"  Actual SCHZ:    {fn:5d}      {tp:5d}")
    print("\n  [Majority Vote Aggregation]")
    print(f"  Accuracy    : {acc_maj:.4f}  ({acc_maj*100:.2f}%)")
    print(f"  Sensitivity : {sens_maj:.4f}  (SCHZ recall)")
    print(f"  Specificity : {spec_maj:.4f}  (CTRL recall)")
    print(f"\n  Confusion Matrix (Majority Vote):")
    print(f"                Pred CTRL  Pred SCHZ")
    print(f"  Actual CTRL:    {tn2:5d}      {fp2:5d}")
    print(f"  Actual SCHZ:    {fn2:5d}      {tp2:5d}")
    print("="*55)
...
def main():
    ...
    labels, preds, probs, subject_ids = get_predictions(model, test_loader, cfg.device)
    print("\n── Slice-level metrics ──")
    metrics = print_metrics(labels, preds, probs)

    print("\n── Subject-level metrics ──")
    subject_results = subject_level_metrics(labels, probs, subject_ids)
